...
module.exports = async (client, entry) => {
    ...
    if(member) await message.channel.send(`:tada: Congratulations ${member.map(member => `<@${member}>`).join(' ') || `<@${member}>`}, you won **${embed.title}** <${message.url}>`);
    if(entry.winners === 1 && member) {
    ...
    if(member) await message.edit(embed);
    if(!member) message.channel.send(`The **${embed.title}** <${message.url}> giveaway has ended but no one won!`);
    entry.update({
